...
if (count($yearsArray) > 0) array_push($filter, 'AND YEAR(date_add) IN (' . implode(', ', $yearsArray) . ')');
...
array_push($where, 'AND ' . db_prefix() . 'cvisit.status > 1');
...
foreach ($rResult as $aRow) {
  ...
  $subjectOutput = '<a href="' . admin_url('operations/view/' . $aRow['id']) . '">' . $aRow['subject'] . '</a>';
  if ($aRow['trash'] == 1) $subjectOutput .= '<span class="label label-danger pull-right">' . _l('contract_trash') . '</span>';
  $subjectOutput .= '<div class="row-options">';
  $subjectOutput .= '<a href="' . admin_url('operations/view/' . $aRow['id']) . '">' . _l('manage_operation') . '</a>';  
  $subjectOutput .= '</div>';
  $row[] = $subjectOutput;
  ...
  $row[] = '<a href="' . admin_url('operations/view/' . $aRow['id']) . '" class="btn btn-xs btn-primary">' . _l('manage_operation') . '</a>';

...
<div class="hide panel_s<?php if(!isset($invoice) || (isset($invoice) && count($invoices_to_merge) == 0 && (isset($invoice) && !isset($invoice_from_project) && count($expenses_to_bill) == 0 || $invoice->status == Invoices_model::STATUS_CANCELLED))){echo ' hide';} ?>" id="invoice_top_info">
   <div class="panel-body">
      <div class="row">
         <div id="merge" class="col-md-6">
            <?php
              if(isset($invoice)) {
                $this->load->view('admin/returns/merge_invoice', array('invoices_to_merge'=>$invoices_to_merge));
              }
            ?>
         </div>
         ...
         <?php if(!isset($invoice_from_project)){ ?>
           <div id="expenses_to_bill" class="col-md-6">
              <?php if(isset($invoice) && $invoice->status != Invoices_model::STATUS_CANCELLED){
                 $this->load->view('admin/returns/bill_expenses',array('expenses_to_bill'=>$expenses_to_bill));
              } ?>
           </div>
         <?php } ?>
      </div>
...
<div class="panel_s invoice accounting-template">
   ...
   <div class="panel-body">
      ...
      <div class="row">
        <div class="col-md-6">
          <div class="f_client_id" style="pointer-events: none;">
            <div class="form-group select-placeholder">
              <label for="clientid" class="control-label"><?php echo _l('invoice_select_customer'); ?></label>
            	...
            	<?php 
             		...
             		if ($selected=="") $selected = (isset($client) ? $client : '');
              	if ($selected == '') $selected = (isset($customer_id) ? $customer_id: '');
              	if ($cliente_id != '') $selected = $cliente_id;
              	if($selected != '') {
          ...
          <div>
          	<a href="#" class="btn btn-info pull-left hidden">
          		<?php echo _l('return_invoices_load'); ?>
          	</a>
          </div>
          <?php
            ...
            <div class="row">
               ...
               <hr class="hr-10" />
                  ...
                  <?php include_once(APPPATH .'views/admin/returns/billing_and_shipping_template.php'); ?>
               </div>
               <div class="col-md-6">
            ...
            <?php
              $next_invoice_number = get_option('next_return_number');
              $format = get_option('invoice_number_format');
              if ($option == "edit") if (isset($invoice)) $format = $invoice->number_format;
              $prefix = get_option('return_prefix');
              ...
              if ($option == "edit") $isedit = isset($invoice) ? 'true' : 'false'; else $isedit = 'false';
              if ($option == "edit") $data_original_number = isset($invoice) ? $invoice->number : 'false'; else $data_original_number = 'false';
            ?>
            <div class="form-group">
               <label for="number"><?php echo _l('return_add_edit_number'); ?></label>
            ...
            <div class="row">
              <div class="col-md-6">
                ...
                ?>
                <?php echo render_date_input('date','return_add_edit_date',$value,$date_attrs); ?>
              </div>
              <div class="col-md-6">
                <?php
                  $value = '';
                ...
                ?>
                <?php echo render_input('duedate','invoice_add_edit_duedate',$value,'text',['readonly'=>'true']); ?>
              </div>
            </div>
                <?php if(is_invoices_overdue_reminders_enabled()){ ?>
               <div class="form-group">
                  <div class="checkbox checkbox-danger">
                     <input type="checkbox" <?php echo 'checked'; ?> id="cancel_overdue_reminders" name="cancel_overdue_reminders" disabled="disabled">
                     <label for="cancel_overdue_reminders"><?php echo _l('cancel_overdue_reminders_return') ?></label>
                  </div>
               </div>
               <?php } ?>
            ...
            <div class="panel_s no-shadow">
              ...
              <div class="form-group">
               ...
               <div class="form-group mbot15 select-placeholder">
                  ...
                  multiple="true"
                  data-width="100%"
                  data-title="<?php echo _l('dropdown_non_selected_tex'); ?>" disabled="disabled">
                  ...
                  <div class="col-md-6">
                    <div class="form-group select-placeholder">
                      ...
                      <select name="discount_type" class="selectpicker" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                        ...
                        <option value="before_tax" <?php if(isset($invoice)) { if ($invoice->discount_type == 'before_tax') { echo 'selected'; }} ?>>
                        	<?php echo _l('discount_type_before_tax'); ?>
                        </option>
                        <option value="after_tax" <?php if (isset($invoice)) { if($invoice->discount_type == 'after_tax') { echo 'selected'; }} ?>>
   ...
   <div class="panel-body mtop10">
      <div class="row">
         <div class="col-md-4">
            <?php $this->load->view('admin/return_items/item_select'); ?>
         </div>
      ...
      <div class="table-responsive s_table">
        <table class="table invoice-items-table items table-main-invoice-edit has-calculations no-mtop">
          ...
          <tbody>
              ...
              <td></td>
              <td>
                <textarea id="description" name="description" class="form-control" rows="4" placeholder="<?php echo _l('item_description_placeholder'); ?>"></textarea>
              </td>
              <td>
                <textarea id="long_description" name="long_description" rows="4" class="form-control" placeholder="<?php echo _l('item_long_description_placeholder'); ?>"></textarea>
              </td>
              ...
              <td>
                <input id="rate" type="number" name="rate" class="form-control" placeholder="<?php echo _l('item_rate_placeholder'); ?>">
              </td>
              <td>
                <?php
                	$dt = $this->invoices_model->get_client_default_tax($cliente_id);
                  $default_tax = unserialize($dt['default_tax']);
              ...
              <td>
                ...
                ?>
                <button id="add_item" type="button" onclick="add_item_to_table('undefined','undefined',<?php echo $new_item; ?>); return false;" class="btn pull-right btn-info"><i class="fa fa-check"></i></button>
              </td>
           	</tr>
               	<?php if (isset($invoice) || isset($add_items) || isset($invoice_items)) {
                  $i               = 1;
                  ...
                  foreach ($add_items as $item) {
                    ...
                    if ($option == "new") $invoice_item_taxes = get_invoice_item_taxes($item['id']);
                    if ($option == "edit") $invoice_item_taxes = get_return_item_taxes($item['id']);
                    ...
                    $select = '<td class="taxrate">';
                      $select .= '<select class="selectpicker display-block tax main-tax" data-width="100%" name="' . $items_indicator . '[' . $i . '][taxname][]" multiple data-none-selected-text="'._l('no_tax').'">';
                      foreach($taxes as $tax) {
                        $selected = '';
                        foreach ($invoice_item_taxes as $item_taxes) {
	                        if (is_array($item_taxes)) {
	                          if(in_array($tax['name'] . '|' . $tax['taxrate'], $item_taxes)) {
	                            $selected = ' selected ';
	                          }
	                        }
                        }
                ...
                <div class="row">
                  ...
                  <div class="col-md-5">
                    <div class="input-group" id="discount-total">
                      ...
                      <input type="number" data-toggle="tooltip" data-title="<?php echo _l('numbers_not_formatted_while_editing'); ?>" value="<?php echo (isset($invoice) ? ($invoice->discount_total*-1) : 0); ?>" class="form-control pull-left input-discount-fixed<?php if(!isset($invoice) || (isset($invoice) && !is_sale_discount($invoice,'fixed'))){echo ' hide';} ?>" min="0" name="discount_total">
                      <div class="input-group-addon">
                        <div class="dropdown">
                          ...
                          <ul class="dropdown-menu" id="discount-total-type-dropdown" aria-labelledby="dropdown_menu_tax_total_type">
                            ...
                            <li>
                            	<a href="#" class="discount-total-type discount-type-fixed<?php if(isset($invoice) && is_sale_discount($invoice,'fixed')){echo ' selected';} ?>"><?php echo _l('discount_fixed_amount'); ?></a>
                            </li>
                          </ul>
                        </div>
                      </div>
   ...
   <div class="row">
      <div class="col-md-12 mtop15">
         <div class="panel-body bottom-transaction">
            ...
            <?php $value = (isset($invoice) ? $invoice->id : ''); ?>
            <?php echo render_input('relinvoice','',$value,'hidden'); ?>

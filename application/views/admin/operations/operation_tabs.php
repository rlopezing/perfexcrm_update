...
<div class="horizontal-scrollable-tabs">
  ...
  <div class="horizontal-tabs">
    <ul class="nav nav-tabs no-margin project-tabs nav-tabs-horizontal" role="tablist">
      <?php
        foreach(filter_operation_visible_tabs($tabs, $operation->settings->available_features) as $key => $tab){
            ...
            <li class="<?php if($key == 'project_overview' && !$this->input->get('group')){echo 'active ';} ?>project_tab_<?php echo $key; ?><?php if($dropdown){echo ' nav-tabs-submenu-parent';} ?>">
              <a
                ...
                <?php if($dropdown){ ?>
                  ...
                  href="<?php echo admin_url('operations/view/' . $operation->id . '?group=' . $key); ?>"
                  <?php } ?>>
                  ...
                  <ul class="dropdown-menu" aria-labelledby="dropdown_<?php echo $key; ?>">
                    <?php
                      foreach($tab['children'] as $d){
                        echo '<li class="nav-tabs-submenu-child"><a href="'.admin_url('operations/view/' . $operation->id . '?group=' . $d['slug']) . '" data-group="' . $d['slug'] . '">' . $d['name'] . '</a></li>';
                      }
                    ?>
                  </ul>
                  <?php if(!is_rtl()){ ?>

...
class Return_model extends App_Model
{
     ...
     * @return array
     */
    public function get($id = '', $option = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($invoice) {
                ...
                if ($option == 'new')	$invoice->items       = get_items_by_type('invoice', $id);
                if ($option == 'edit')	$invoice->items     = get_items_by_type('return', $id);
    ...
    public function add($data, $expense = false)
    {
    	...
    	$data['subtotal'] = $data['subtotal'] * -1;
    	$data['total'] = $data['total'] * -1;
    	$data['discount_total'] = $data['discount_total'] * -1;
    	
      $data['prefix'] = get_option('return_prefix');
      ...
      if (isset($data['relinvoice'])) $relinvoice = $data['relinvoice'];
      ...
      if ($insert_id) {
          ...
          $this->db->where('name', 'next_return_number');
          ...
          // Update status invoice
  				$invoice = $this->get($relinvoice);
  				if (($invoice->total-$total) == 0 ) {
            $this->db->where('id', $relinvoice);
            $this->db->update(db_prefix() . 'invoices', [
          ...
          foreach ($items as $key => $item) {
              if ($itemid = add_new_sales_item_post($item, $insert_id, 'return')) {
                  ...
                  _maybe_insert_post_item_tax($itemid, $item, $insert_id, 'return');
              }
          }
          
          $contracts = explode(",", $contracts);
          foreach ($contracts as $key => $contract) {
            $contractid = update_contract_post($contract,1);
          }

          update_return_total_tax_column($insert_id, 'return', db_prefix() . 'invoices');
    ...
    public function update($data, $id)
    {
	    ...
	    if (count($items) > 0) {
	        foreach ($items as $key => $item) {
	            ...
	            if (!isset($item['taxname']) || (isset($item['taxname']) && count($item['taxname']) == 0)) {
	                if (delete_taxes_from_item($item['itemid'], 'return')) {
	                    $affectedRows++;
	                }
	            } else {
	                $item_taxes        =  get_return_item_taxes($item['itemid']);
	                $_item_taxes_names = [];
	                ...
	                if (_maybe_insert_post_item_tax($item['itemid'], $item, $id, 'return')) {
	                    $affectedRows++;
	                }
	            }
	        }
	    ...
	    foreach ($newitems as $key => $item) {
	        if ($new_item_added = add_new_sales_item_post($item, $id, 'invoice')) {
	            ...
	            _maybe_insert_post_item_tax($new_item_added, $item, $id, 'return');
	    ...
	    if ($affectedRows > 0) {
	      update_sales_total_tax_column($id, 'return', db_prefix() . 'invoices');
  ...
  public function get_contract($id) {
  	$this->db->select("tblcomicontratos.id, tblclients.company as cliente, tblcomicontratos.cpe, tblcomitarifa.descripcion as tarifa, tblcominivelprecio.detalle as nivel_precios, tblcomicomercializador.nombre as comercializador, tblcomicontratos.valor_contrato, tblcomicontratos.comision_emsconsulting, tblcomicontratos.comision_socio, tblcomicontratos.comision_comercial");
    $this->db->join("tblclients","tblclients.userid = tblcomicontratos.cliente");
    $this->db->join("tblcomitarifa","tblcomitarifa.id = tblcomicontratos.tarifa");
    $this->db->join("tblcominivelprecio","tblcominivelprecio.id = tblcomicontratos.nivel_precios");
    $this->db->join("tblcomicomercializador","tblcomicomercializador.id = tblcomicontratos.comercializador");
    $this->db->where('tblcomicontratos.id', $id);

    return $this->db->get('tblcomicontratos')->row();
  }

  public function get_invoiced_contracts($id) {
  	$this->db->select("contracts");
    $this->db->where('id', $id);

    return $this->db->get('tblinvoices')->row();
  }
  
  public function get_list_clients($userid = '') {

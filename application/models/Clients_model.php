...
class Clients_model extends App_Model
{
    ...
    public function get_types($id = '')
    {
      ...
      return $this->db->get(db_prefix() . 'cvtype')->result_array();
    }
    ...
    public function add($data, $client_or_lead_convert_request = false)
    {
        ...
        $data['default_tax'] = serialize($data['default_tax']);
   ...
   * @return array
   */
  public function get_supply_points($cliente = "", $id = "")
  {
		$this->db->select("tblsupplypoints.*, tblmodule.name as module, tblcountries.short_name");
    $this->db->join(db_prefix().'module', db_prefix().'module.id = '.db_prefix().'supplypoints.module_id');
    $this->db->join(db_prefix().'countries', db_prefix().'countries.country_id = '.db_prefix().'supplypoints.country');
    $this->db->where(db_prefix().'supplypoints.cliente', $cliente);
    if($id != "") $this->db->where(db_prefix().'supplypoints.id', $id);
    
    return $this->db->get(db_prefix().'supplypoints')->result_array();
  }
  ...
  */
  public function add_supply_points($data) {
    $this->db->insert('tblsupplypoints', $data);
   ...
   */
  public function update_supply_points($data, $id) {
    $this->db->where('id', $id);
    $this->db->update('tblsupplypoints', $data);
    if ($this->db->affected_rows() > 0) {
      return true;
    }

...
class Commissions extends Admin_controller
{
    ...
    public function index() {
      close_setup_menu();
      if (!has_permission('commissions', '', 'view') && !has_permission('commissions', '', 'view_own')) access_denied('commissions');
      ...
      $data['list_clients']				= $this->commissions_model->get_clientes_invoice();
      $data['title']         			= _l('contracts');
      
      $this->load->view('admin/commission/manage', $data);
    }
    
    public function table($clientid = '') {
      if (!has_permission('commissions', '', 'view') && !has_permission('commissions', '', 'view_own')) ajax_access_denied();
      $this->app->get_table_data('comicontracts', ['clientid' => $clientid,]);
    }
    ...
    public function contract($id = '') {
      if ($this->input->post()) {
        if ($id == ''){
          ...
          $id = $this->commissions_model->add($this->input->post());
          if ($id) {
          ...
          $success = $this->commissions_model->update($this->input->post(), $id);
          if ($success) {
      ...
      if ($id == '') {
        ...
        $data['contract']                 	= $this->commissions_model->get($id, [], true);
        ...
        $data['accion'] 										= "edit";
        $data['termination_motive'] 				= $this->commissions_model->get_contract_termination_motive();
      }

...
class Invoices extends AdminController {
  ...
  public function list_invoices($id = '') 
  {
    ...
    $data['list_clients']    			= $this->invoices_model->get_list_clients('');
    $data['cliente_id'] 					= "";
    $_SESSION['cliente_id'] 			= "";
  ...
  public function table($clientid = '') {
      ...
      if ($this->input->post()) 
      {
        ...
        if (count($invoice_data) > 4) {
	        if ($id == '') {
	          ...
	          if (!has_permission('invoices', '', 'edit')) access_denied('invoices');
	          $success = $this->invoices_model->update($invoice_data, $id);
	          if ($success) set_alert('success', _l('updated_successfully', _l('invoice')));
	          redirect(admin_url('invoices/list_invoices/' . $id));
	        }
        } 
        
        // FACTURADOR DE COMISIONES.
        if (count($invoice_data) == 4) {
					$cliente = $this->commissions_model->get_clientes_invoice($invoice_data["cliente"]);
					$invoice_data["cliente"] 		= $cliente->cliente;
					$data['cliente_id'] 				= $cliente->cliente;
					$data['customer_id'] 				= $cliente->cliente;
			  	
			    $title                  		= _l('create_new_invoice');
			    $data['billable_tasks'] 		= [];
			    $data['client'] 						= $invoice_data['cliente'];
			    $data['expenses_to_bill']  	= [];
			    $data['invoices_to_merge'] 	= [];
			    
			    //Obtener listado de contratos.
			    $invoice = (object) array(
			    	"clientid" => $invoice_data['cliente'],
			    	"items" 	 => array()
			    );
			    
			    $description = "Comisiones desde ".$invoice_data['fdesde']." hasta ".$invoice_data['fhasta'].".";
			    $long_description = "CUPS/CPES: ";
			    $rate = 0;
			    $items = [];
			    $contracts = explode(",",$invoice_data['contracts']);
			    
			    foreach ($contracts as $contract) {
			    	$get_contracts = $this->invoices_model->get_contract($contract);
			    	$long_description = $long_description." ".$get_contracts->cpe.", ";
			    	$rate = $rate + $get_contracts->valor_contrato;
					}
			    ...
			    $data['contracts']        = $invoice_data['contracts'];
	      ...
	      if ($this->input->get('customer_id')) {
	        $data['customer_id'] = $this->input->get('customer_id');
	        $data['cliente_id'] = $this->input->get('customer_id');
	      }
	      $data['payment_modes'] = $this->payment_modes_model->get('', [
	        'expenses_only !=' => 1,
	      ]);
    ...
    public function delete($id)
    {
        ...
        $contracts = $this->invoices_model->get_invoiced_contracts($id);

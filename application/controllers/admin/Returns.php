...
class Returns extends AdminController 
{
  ...
  {
    if (!has_permission('returns', '', 'view') && !has_permission('returns', '', 'view_own') && get_option('allow_staff_view_invoices_assigned')=='0') {
      access_denied('returns');
    }
    
    close_setup_menu();
    
    $data['title']                = _l('returns');
    $data['payment_modes']        = $this->payment_modes_model->get('', [], true);
    $data['invoiceid']            = $id;
    $data['invoices_years']       = $this->return_model->get_invoices_years();
    $data['invoices_sale_agents'] = $this->return_model->get_sale_agents();
    $data['invoices_statuses']    = $this->return_model->get_statuses();
    $data['bodyclass']            = 'invoices-total-manual';
    $data['list_clients']    			= $this->return_model->get_list_clients('');
    $data['cliente_id'] 					= "";
    $_SESSION['cliente_id'] 			= "";
    
    $this->load->view('admin/returns/manage', $data);
  }
  
  /* List all invoices datatables */
  public function list_contracts($data_invoice,$id='') 
  {
    if (!has_permission('returns', '', 'view') && !has_permission('returns', '', 'view_own') && get_option('allow_staff_view_invoices_assigned') == '0') {
      access_denied('returns');
    }
    ...
    $this->invoice_contracts($data_invoice,$id);
  }
  ...
  public function recurring($id = '') 
  {
    ...
    $this->load->view('admin/returns/recurring/list', $data);
  }
  ...
  public function table($clientid = '') 
  {
    ...
    $data['payment_modes'] = $this->payment_modes_model->get('', [], true);
    $this->app->get_table_data(($this->input->get('recurring') ? 'recurring_invoices' : 'returns'), [
      'clientid' => $clientid,
      'data'     => $data,
    ]);
  }
  ...
  public function client_change_data($customer_id, $current_invoice = '')
  {
    if ($this->input->is_ajax_request()) {
      ...
      $data['merge_info'] = $this->load->view('admin/returns/merge_invoice', $_data, true);
      $this->load->model('currencies_model');
      ...
      $data['expenses_bill_info'] = $this->load->view('admin/returns/bill_expenses', $__data, true);
    ...
    public function update_number_settings($id)
    {
        ...
        if (has_permission('returns', '', 'edit')) {
            $affected_rows = 0;
    ...
    {
        if (!has_permission('returns', '', 'edit') && !has_permission('returns', '', 'create')) {
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        $success = $this->return_model->unmark_as_cancelled($id);
        if ($success) {
    ...
    public function copy($id)
    {
        ...
        if (!has_permission('returns', '', 'create')) {
            access_denied('returns');
        }
        $new_id = $this->return_model->copy($id);
        if ($new_id) {
        ...
        redirect(admin_url('returns/invoice/' . $id));
    }

    public function get_merge_data($id)
    {
        $invoice = $this->return_model->get($id);
    ...
    public function invoice($id = '', $option = '') 
    {
      if ($this->input->post()) {
        ...
        if (count($invoice_data) > 4) {
        	if (!has_permission('returns', '', 'create')) access_denied('invoices');
	        if ($id == '') {
	          if (!has_permission('returns', '', 'create')) access_denied('invoices');
	          $id = $this->return_model->add($invoice_data);
	          if ($id) {
	            set_alert('success', _l('added_successfully', _l('return')));
	            $redUrl = admin_url('returns/list_invoices/' . $id);
	            if (isset($invoice_data['save_and_record_payment'])) $this->session->set_userdata('record_payment', true);
	            redirect($redUrl);
	          }
	        } else {
	          if (!has_permission('returns', '', 'edit')) access_denied('invoices');
	          $success = $this->return_model->update($invoice_data, $id);
	          if ($success) set_alert('success', _l('updated_successfully', _l('return')));
	          redirect(admin_url('returns/list_invoices/' . $id));
	        }
        } 
        
      } else {
			
        $title                  		= _l('sales_return_new');
        $data['billable_tasks'] 		= [];
        $invoice 										= $this->return_model->get($id,$option);
        $data['id'] 								= $id;
        $data['cliente_id'] 				= $invoice->clientid;
        $invoice->prefix 						= get_option('return_prefix');
        
        if (!$invoice || !user_can_view_invoice($id)) blank_page(_l('invoice_not_found'));
        
        $data['invoices_to_merge'] 	= $this->return_model->check_for_merge_invoice($invoice->clientid, $invoice->id);
        $data['expenses_to_bill']  	= $this->return_model->get_expenses_to_bill($invoice->clientid);
        ...
        $data['billable_tasks'] 		= $this->tasks_model->get_billable_tasks($invoice->clientid, !empty($invoice->project_id) ? $invoice->project_id : '');
	      ...
	      $this->load->view('admin/returns/invoice', $data);
      }
    }
  ...
  public function get_invoice_data_ajax($id)
  {
    ...
    if (!$id) die(_l('invoice_not_found'));
    
    $invoice = $this->return_model->get($id, "edit");
    ...
    if ($invoice->sent == 1) $template_name = 'invoice_send_to_customer_already_sent';
    $data = prepare_mail_preview_data($template_name, $invoice->clientid);

    // Check for recorded payments
    $this->load->model('payments_model');
    //$data['invoices_to_merge']          = $this->return_model->check_for_merge_invoice($invoice->clientid, $id);
    $data['members']                    = $this->staff_model->get('', ['active' => 1]);
    $data['payments']                   = $this->payments_model->get_invoice_payments($id);
    $data['activity']                   = $this->return_model->get_invoice_activity($id);
    $data['totalNotes']                 = total_rows(db_prefix().'notes', ['rel_id' => $id, 'rel_type' => 'invoice']);
    $data['invoice_recurring_invoices'] = $this->return_model->get_invoice_recurring_invoices($id);
    ...
    $this->load->view('admin/returns/invoice_preview_template', $data);
  }
    ...
    public function send_to_email($id)
    {
        ...
        if (!$canView) {
            ...
            if (!has_permission('returns', '', 'view') && !has_permission('returns', '', 'view_own') && $canView == false) {
                access_denied('Returns');
            }
        }

        try {
            $success = $this->return_model->send_invoice_to_client($id, '', $this->input->post('attach_pdf'), $this->input->post('cc'));
        } catch (Exception $e) {
    ...
    {
        if (!has_permission('returns', '', 'delete')) {
            access_denied('returns');
        }
        if (!$id) {
            redirect(admin_url('returns/list_invoices'));
        }
        $contracts = $this->return_model->get_invoiced_contracts($id);
        $success = $this->return_model->delete($id);
        
        if ($success) {
        	// reactiva contratos para ser facturados nuevamente
        	if (!is_null($contracts->contracts)){
            $contracts = explode(",", $contracts->contracts);
            foreach ($contracts as $key => $contract) {
              $contractid = update_contract_post($contract,0);
            }
					}
    ...
    public function delete_attachment($id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            echo $this->return_model->delete_attachment($id);
        } else {
    ...
    public function send_overdue_notice($id)
    {
        ...
        $send = $this->return_model->send_invoice_overdue_notice($id);
        if ($send) {
    ...
    public function mark_as_sent($id)
    {
        ...
        $success = $this->return_model->set_invoice_sent($id, true);
        if ($success) {
  ...
  public function get_client_default_tax($userid = "") {
  	$default_tax = $this->invoices_model->get_client_default_tax($userid);
  	echo json_encode($default_tax);
  }

...
class Clients extends AdminController
{
    ...
    public function client($id = '')
    {
        ...
        $data['types']      = $this->clients_model->get_types(); 
  ...
  public function delete_supply_points($id="",$cliente="") {
    if (!has_permission('customers', '', 'create') && !has_permission('customers', '', 'edit')) {
      access_denied('customers');
    }
    $this->db->where('id', $id);
    $this->db->delete(db_prefix().'supplypoints');

    redirect(admin_url('clients/client/'.$cliente));
  }
  ...
  /* Edit or add new supply points */
  public function supply_points($id) {
    if ($this->input->post()) {
      $data = $this->input->post();
      $userid = $data["cliente"];
      ...
      if ($id == undefined) {
        ...
        $id = $this->clients_model->add_supply_points($data);
        if ($id) {
          set_alert('success', _l('added_successfully', _l('client')));
          $success = true;
        ...
        $success = $this->clients_model->update_supply_points($data, $id);
        if ($success == true) {
          set_alert('success', _l('updated_successfully', _l('client')));
          $success = true;
          $message = _l('added_successfully', _l('client'));
  ...
  public function get_supply_points($cliente = "", $id = '') {
		$supplypoints = $this->clients_model->get_supply_points($cliente, $id);
		echo json_encode($supplypoints);
	}

...
class App {
    ...
    public function get_table_data($table, $params = []) {
        ...
        // Filtros para tarifas
				if (!isset($_SESSION['country_id']) || $_SESSION['country_id']=="") $country_id=0; else $country_id=$_SESSION['country_id'];
				if (!isset($_SESSION['module_id']) || $_SESSION['module_id']=="") $module_id=0; else $module_id=$_SESSION['module_id'];
        
        $plan = $_SESSION['plan'];
        $cliente = $_SESSION['cliente'];
        
        // Filtros para el mapa general.
        if ($_SESSION['$fdesde']!='' && $_SESSION['$fhasta']!=''){
			    $fdesde = $_SESSION['$fdesde'];
			    $fhasta = $_SESSION['$fhasta'];
				}
				
				// Filtros tabla de tarifas simulador.
        if ($_SESSION['$rate_id']!='') $rate_id = $_SESSION['$rate_id'];
        if ($_SESSION['$marketer_id']!='') $marketer_id = $_SESSION['$marketer_id'];
        
        // Filter for employee
        if ($_SESSION['staffid']!='') $staffid = $_SESSION['staffid'];

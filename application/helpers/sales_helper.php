...
/**
* Function that update total tax in returns table eq. return, proposal, estimates, credit note
* @param  mixed $id
* @return void
*/
function update_return_total_tax_column($id, $type, $table)
{
    $CI = &get_instance();
    $CI->db->select('discount_percent, discount_type, discount_total, subtotal');
    $CI->db->from($table);
    ...
    $func_taxes = 'get_' . $type . '_item_taxes'; // get_invoice_item_taxes
    foreach ($items as $item) 
    {
        $item_taxes = call_user_func($func_taxes, $item['id']);
        if (count($item_taxes) > 0) {
    ...
    $CI->db->where('id', $id);
    $CI->db->update($table, ['total_tax' => $total_tax,]);
}
 ...
 * @return boolean
 */
function update_contract_post($contract_id,$value) {
  $update = [];
  $update = ['invoiced' => $value];

  $CI = &get_instance();
  $CI->db->where('id', $contract_id);
  $CI->db->update(db_prefix() . 'comicontratos', $update);
...
if (!function_exists('format_simulator_info')) {
  /**
   * Format simulator info format
   * @param  object $simulator simulator from database
   * @param  string $for      where this info will be used? Admin area, HTML preview?
   * @return string
   */
  function format_simulator_info($simulator, $for = '')
  {
    $format = get_option('simulator_info_format');

    $countryCode = '';
    $countryName = '';
    $country     = get_country($simulator->country);
    ...
    $contactTo = '<b>'.$simulator->firstname.' '.$simulator->lastname.'</b>';
    $phone      = $simulator->phonenumber;
    $email      = $simulator->email;

    if ($for == 'admin') {
      $hrefAttrs = '';
      if ($simulator->rel_type == 'lead') {
        $hrefAttrs = ' href="#" onclick="init_lead('.$simulator->rel_id.'); return false;" data-toggle="tooltip" data-title="'._l('lead').'"';
      } else {
        $hrefAttrs = ' href="'.admin_url('simulators/simulator/'.$simulator->rel_id).'" data-toggle="tooltip" data-title="'._l('client').'"';
      }
      $contactTo = '<a'.$hrefAttrs.'>'.$contactTo.'</a>';
    }

    if ($for == 'html' || $for == 'admin') {
      $phone = '<a href="tel:' . $simulator->phonenumber . '">' . $simulator->phonenumber . '</a>';
      $email = '<a href="mailto:' . $simulator->email . '">' . $simulator->email . '</a>';
    }

    $format = _info_format_replace('contact', $contactTo, $format);
    $format = _info_format_replace('address', $simulator->address, $format);
    $format = _info_format_replace('city', $simulator->city, $format);
    $format = _info_format_replace('state', $simulator->state, $format);

    $format = _info_format_replace('country_code', $countryCode, $format);
    $format = _info_format_replace('country_name', $countryName, $format);

    $format = _info_format_replace('zip_code', $simulator->zip, $format);
    $format = _info_format_replace('phone', $phone, $format);
    $format = _info_format_replace('email', $email, $format);
    ...
    $customFieldsProposals = get_custom_fields('simulator', $whereCF);

    foreach ($customFieldsProposals as $field) {
      $value  = get_custom_field_value($simulator->id, $field['id'], 'simulator');
      $format = _info_format_custom_field($field['id'], $field['name'], $value, $format);
    }

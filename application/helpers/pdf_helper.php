...
{
    return app_pdf('return', LIBSPATH . 'pdf/Invoice_pdf', $invoice, $tag);
}

/**
 * Prepare general retrun pdf
 * @param  object $invoice Retrun as object with all necessary fields
 * @param  string $tag     tag for bulk pdf exporter
 * @return mixed object
 */
function return_pdf($invoice, $tag = '')
{
    return app_pdf('invoice', LIBSPATH . 'pdf/Return_pdf', $invoice, $tag);
}
 ...
 * @param  object $simulator object db
 * @return mixed object
 */
function simulation_pdf($simulator)
{
  $GLOBALS['simulator_pdf'] = $simulator;
  $CI                      = & get_instance();
  $CI->load->library('pdf');
  $formatArray = get_pdf_format('pdf_format_contract');
  
  if (!file_exists(APPPATH . 'libraries/Simulator_pdf.php')) {
    $pdf = new Pdf($formatArray['orientation'], 'mm', $formatArray['format'], true, 'UTF-8', false, false, 'contract');
  } else {
    include_once(APPPATH . 'libraries/Simulator_pdf.php');
    $pdf = new Simulation_pdf($formatArray['orientation'], 'mm', $formatArray['format'], true, 'UTF-8', false, false);
  }
		
  $font_name = get_option('pdf_font');
  $font_size = get_option('pdf_font_size');
  if ($font_size == '') $font_size = 10;
  ...
  $pdf->SetTitle(_l("simulation_content"));
...
/*    # Don't remove these lines - important for the PDF layout
  ...
  $simulator->content = preg_replace('/(<img[^>]+>(?:<\/img>)?)/i', '<div>$1</div>', $simulator->content);
  // Add cellpadding to all tables inside the html
  $simulator->content = preg_replace('/(<table\b[^><]*)>/i', '$1 cellpadding="4">', $simulator->content);
  // Remove white spaces cased by the html editor ex. <td>  item</td>
  $simulator->content = preg_replace('/[\t\n\r\0\x0B]/', '', $simulator->content);
  $simulator->content = preg_replace('/([\s])\1+/', ' ', $simulator->content);

  // Tcpdf does not support float css we need to adjust this here
  $simulator->content = str_replace('float: right', 'text-align: right', $simulator->content);
  $simulator->content = str_replace('float: left', 'text-align: left', $simulator->content);
  // Image center
  $simulator->content = str_replace('margin-left: auto; margin-right: auto;', 'text-align:center;', $simulator->content);
*/

  if (file_exists(APPPATH.'views/themes/'.active_clients_theme().'/views/my_simulatorpdf.php')) {
    include(APPPATH.'views/themes/'.active_clients_theme().'/views/my_simularotpdf.php');
  } else {
    include(APPPATH.'views/themes/'.active_clients_theme().'/views/simulatorpdf.php');
  }
...
function gas_pdf($simulator)
{
  ...
  if (!file_exists(APPPATH . 'libraries/gas_pdf.php')) {
    $pdf = new Pdf($formatArray['orientation'], 'mm', $formatArray['format'], true, 'UTF-8', false, false, 'contract');
  } else {
    include_once(APPPATH . 'libraries/gas_pdf.php');
    $pdf = new Simulation_pdf($formatArray['orientation'], 'mm', $formatArray['format'], true, 'UTF-8', false, false);
  }
  ...
  if (file_exists(APPPATH.'views/themes/'.active_clients_theme().'/views/my_gaspdf.php')) {
    include(APPPATH.'views/themes/'.active_clients_theme().'/views/my_gaspdf.php');
  } else {
    include(APPPATH.'views/themes/'.active_clients_theme().'/views/gaspdf.php');
  }

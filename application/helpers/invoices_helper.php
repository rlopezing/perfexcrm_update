...
function format_invoice_status($status, $classes = '', $label = true)
{
    ...
    } elseif ($status == Invoices_model::STATUS_DRAFT) {
        $status = _l('invoice_status_draft');
    } elseif ($status == Invoices_model::STATUS_RETURN) {
			$status = _l('invoice_status_return');
		}
    if ($label == true) {
...
function get_invoice_status_label($status)
{
    ...
    if ($status == Invoices_model::STATUS_UNPAID) {
        ...
        $label_class = 'warning';
    } elseif ($status == Invoices_model::STATUS_CANCELLED || $status == Invoices_model::STATUS_DRAFT || $status == Invoices_model::STATUS_RETURN) {
        $label_class = 'default';
    } else {
...
/**
 * Format return number based on description
 * @param  mixed $id
 * @return string
 */
function format_rel_invoice($id)
{
    $CI = & get_instance();
    $CI->db->select('date,number,prefix,number_format')->from(db_prefix() . 'invoices')->where('id', $id);
    $invoice = $CI->db->get()->row();
...
/**
 * Function that return returns item taxes based on passed item id
 * @param  mixed $itemid
 * @return array
 */
function get_return_item_taxes($itemid)
{
    $CI = & get_instance();
    $CI->db->where('itemid', $itemid);
    $CI->db->where('rel_type', 'return');
    $taxes = $CI->db->get(db_prefix() . 'item_tax')->result_array();
    $i     = 0;
    foreach ($taxes as $tax) {
        $taxes[$i]['taxname'] = $tax['taxname'] . '|' . $tax['taxrate'];
